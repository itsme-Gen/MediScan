...
const SignUp: React.FC = () => {
  ...
  const validateStep1 = () => {
    ...
    if (
      ...
      if (!toastActive) {
        setToastActive(true);
        toast.error("Please fill out all required fields", { onClose: () => setToastActive(false) });
      }
      return false;
    }
    return true;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      if (!toastActive) {
        setToastActive(true);
        toast.error("Password does not match", { onClose: () => setToastActive(false) });
      }
      return;
    }
    ...
    try {
      ...
      if (!toastActive) {
        setToastActive(true);
        toast.error("Registration failed", { onClose: () => setToastActive(false) });
      }
    }
  }
            ...
            <>
              ...
              <div className="personal_information w-full grid grid-cols-2 gap-4">
                ...
                <FormControl sx={{ m: 0, minWidth: 80 }}>
                  ...
                  <Select labelId="gender-label" name='gender' value={formData.gender} onChange={handleChange} autoWidth label="Gender">
              ...
              <div className="professional_info_container">
                ...
                <div className="professional_information w-full grid grid-cols-2 gap-4">
                  ...
                  <TextField required type='text' label="Department e.g. Cardiology" name='department' value={formData.department} onChange={handleChange} variant='outlined' fullWidth />
                  <TextField required type='number' name='licenseNumber' value={formData.licenseNumber} onChange={handleChange} label="Professional License Number" variant='outlined' fullWidth />
